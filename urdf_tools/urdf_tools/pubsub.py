...
class SensorSubscriber:
    ...
    def detach(self) -> None:
        # ExtShmBridge.close() unmaps without unlinking when called by subscriber
        # We call close but the publisher owns the segment lifetime.
        try:
            if self._ext._mm is not None:
                self._ext._mm.close()
                self._ext._mm = None
        except Exception:
            pass
